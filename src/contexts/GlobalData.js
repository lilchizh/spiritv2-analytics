...
const UPDATE_ALL_PAIRS_IN_HYPERSWAP = 'UPDATE_ALL_PAIRS_IN_HYPERSWAP'
const UPDATE_ALL_TOKENS_IN_HYPERSWAP = 'UPDATE_ALL_TOKENS_IN_HYPERSWAP'
...
function reducer(state, { type, payload }) {
  switch (type) {
    ...
    case UPDATE_ALL_PAIRS_IN_HYPERSWAP: {
    ...
    case UPDATE_ALL_TOKENS_IN_HYPERSWAP: {
...
export default function Provider({ children }) {
  ...
  const updateAllPairsInHyperswap = useCallback((allPairs) => {
    dispatch({
      type: UPDATE_ALL_PAIRS_IN_HYPERSWAP,
      payload: {
        allPairs,
      },
    })
  ...
  const updateAllTokensInHyperswap = useCallback((allTokens) => {
    dispatch({
      type: UPDATE_ALL_TOKENS_IN_HYPERSWAP,
      payload: {
        allTokens,
      },
    })
      ...
      value={useMemo(
        () => [
          state,
          {
            ...
            updateFtmPrice,
            updateTopLps,
            updateAllPairsInHyperswap,
            updateAllTokensInHyperswap,
          },
        ],
        [
          state,
...
async function getGlobalData(ftmPrice, oldFtmPrice, isV3, client) {
  ...
  try {
    ...
    if (data && oneDayData && twoDayData) {
      ...
      data.totalLiquidityUSD = data.totalLiquidityETH * ftmPrice
      const liquidityChangeUSD = getPercentChange(
        data.totalLiquidityETH * ftmPrice,
...
export function useAllPairsInHyperswap() {
...
export function useAllTokensInHyperswap() {
  const [state] = useGlobalDataContext()

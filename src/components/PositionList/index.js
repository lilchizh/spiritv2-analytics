...
const DashGrid = styled.div`
  ...
  grid-template-areas: 'number name hyperswap return';
  ...
  @media screen and (max-width: 740px) {
    ...
    grid-template-areas: 'name hyperswap return';
  }

  @media screen and (max-width: 500px) {
    grid-template-columns: 2.5fr 1fr;
    grid-template-areas: 'name hyperswap';
  }
`
...
const SORT_FIELD = {
  VALUE: 'VALUE',
  HYPERSWAP_RETURN: 'HYPERSWAP_RETURN',
}
                ...
                <RowFixed>
                  <TYPE.small fontWeight={400}>
                    {parseFloat(position.pair.token0.derivedETH)
                      ? formattedNum(
                          position?.fees.sum / (parseFloat(position.pair.token0.derivedETH) * ftmPrice) / 2,
                          false,
                          true
                        )
                      : 0}{' '}
                ...
                <RowFixed>
                  <TYPE.small fontWeight={400}>
                    {parseFloat(position.pair.token1.derivedETH)
                      ? formattedNum(
                          position?.fees.sum / (parseFloat(position.pair.token1.derivedETH) * ftmPrice) / 2,
                          false,
                          true
                        )
                      : 0}{' '}
  ...
  const positionsSorted =
    ...
    positions
      ...
      .sort((p0, p1) => {
        ...
        if (sortedColumn === SORT_FIELD.HYPERSWAP_RETURN) {
          return p0?.hyperswap?.return > p1?.hyperswap?.return ? (sortDirection ? -1 : 1) : sortDirection ? 1 : -1
        }
        ...
        {!below500 && (
          <Flex alignItems="center" justifyContent="flexEnd">
            <ClickableText
              area="return"
              onClick={() => {
                setSortedColumn(SORT_FIELD.HYPERSWAP_RETURN)
                setSortDirection(sortedColumn !== SORT_FIELD.HYPERSWAP_RETURN ? true : !sortDirection)
              }}
            >
              {below740 ? 'Fees' : 'Total Fees Earned'}{' '}
              {sortedColumn === SORT_FIELD.HYPERSWAP_RETURN ? (!sortDirection ? '↑' : '↓') : ''}
            </ClickableText>
          </Flex>
        )}
      </DashGrid>

...
import { useAllPairsInHyperswap, useAllTokensInHyperswap } from '../../contexts/GlobalData'
...
const Container = styled.div`
  ...
  z-index: 30;
  position: relative;
  border: 1px solid rgb(82, 215, 132);
  border-radius: 12px;
...
export const Search = ({ small = false }) => {
  let allTokens = useAllTokensInHyperswap()
  const allTokenData = useAllTokenData()

  let allPairs = useAllPairsInHyperswap()
        ...
        <Input
          ...
          placeholder={
            small
              ...
              : 'Search SpiritSwap pairs and tokens...'
          }
          value={value}
          onChange={(e) => {
        ...
        <div>
          ...
          {filteredPairList &&
            filteredPairList.slice(0, pairsShown).map((pair) => {
              if (pair?.token0?.id === '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83') {
                pair.token0.name = 'FTM (Wrapped)'
                pair.token0.symbol = 'FTM'
              }
              if (pair?.token1.id === '0x21be370d5312f44cb42ce377bc9b8a0cef1a4c83') {
                pair.token1.name = 'FTM (Wrapped)'
